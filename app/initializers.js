/**
 * Created by Rajesh on 7/1/19. Modified by Zihad Ul Islam Mahdi on 9/9/19. 
 */
            ...
            User.find({email: systemInfo.email}, function (err, result) {
                if(err){
                    ...
                    User.create(userRequest, function (err, response) {
                        if(err){
                            console.log("System User could not be created, MONGO DB failed!!!");
                        }else{
                            console.log("System User Created with email: " + systemInfo.email);
                        }
                    })
                }
            });
                ...
                if(count == 0){
                    defaultDataTypes.map(function (dataType) {
                        DataType.model.create(dataType, function (err, resp) {
                            if(err){
                                console.log("Default Data Type Could not be created, MONGO DB failed!!!")
                            }else{
                                console.log("Data type Created with type: " + dataType.dataType);
                            }
                        })
                    });
                }else{
