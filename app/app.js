...
mongoose.connect('mongodb://' + mongoHost + ':' + mongoPort + '/' + database, {useNewUrlParser: true});
initializers.createSystemUser();
initializers.createFieldDataType();

app.use("/api/", routes);

app.get('/dashboard*', [middleware.requireAuthentication, middleware.superAdminAuth], function(req, res){
    res.render('index');
});
