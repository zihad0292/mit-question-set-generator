/**
 * Created by Rajesh on 6/12/19. Modified by Zihad Ul Islam Mahdi on 9/2/19. added param office to fetchofficelist
 */
...
export function addNewOffice(name, location) {
    return function (dispatch) {
        ...
        axios.post(`${baseUrl}new?n=${name}&l=${location}`)
            .then((response) => {
                const d = response.data;
    ...
    return function (dispatch) {
        ...
        axios.post(updateUrl)
            .then((resp) => {
                ...
                if (d.success) {
                    dispatch({type: "UPDATING_OFFICE_INFO_FULFILLED", payload: d.message});
                } else {
        ...
        axios.delete(`${baseUrl}delete?id=${id}`)
            .then((resp) => {
                ...
                if(d.success){
                    ...
                    dispatch({type: "DELETE_OFFICE_INFO_FULFILLED", payload: d.message});
                }else{
                    dispatch({type: "FETCHING_OFFICE_LIST_FAILED", payload: d.error});
                }
            })
