...
class AddNewFtp extends Component{
    constructor(props){
        super(props);
        this.state = {
            form: 'ftp',
    ...
    populateStateVal(){
        ...
        this.setState({
            ftpName: selectedData.credentialName,
            server: selectedData.host,
    ...
    //     const {ftpServer, ftpPort, ftpUser, ftpPass, connection} = this.state;
    ...
    //     checkFtpConnection(ftpServer, ftpPort, ftpUser, ftpPass).then((resp) => {
    //             if(resp.success){
    //                 tempConn = {
    //                     status: true,
    //                     checking: false,
    ...
    handleSubmit(){
        const {form, ftpName, server, port, user, pass, enable, homeDirectory, connection} = this.state;
        const {type, selectedData, addNewCredential, updateCredential, selectedDataType} = this.props;

        if(ftpName === '' || server === '' || port === '' || user === '' || pass === ''){
            this.setState({
                showErrorMessage: true
            });
        }else if(type == 'update'){
            updateCredential(selectedData._id, form, ftpName, selectedDataType, server, port, user, pass, enable, '', '', homeDirectory);
        }else {
            addNewCredential(form, ftpName, selectedDataType, server, port, user, pass, enable, '', '', homeDirectory);
        }
        ...
        return(
            <Grid container direction="row" justify="flex-start" alignItems="center" style={{marginTop: '7px'}}>
                <Button size="small" variant="outlined" color="secondary"
                        disabled={ this.state.server.length == 0 || this.state.port.length == 0 || this.state.user.length == 0 || this.state.pass.length == 0 }
        ...
        return(
                <div>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <TextField
                                id="ftp-name" name="ftpName" label="Name" value={this.state.ftpName}
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={8}>
                            <TextField
                                id="ftp-server" name="server" label="FTP Server" value={this.state.server}
                                margin="normal" variant="outlined" fullWidth
                                onChange={this.handleChange}
                            />
                        </Grid>
                        <Grid item xs={4}>
                            <TextField
                                id="ftp-port" name="port" label="Port" value={this.state.port}
                                margin="normal" variant="outlined" fullWidth
                                onChange={this.handleChange}
                            />
                        </Grid>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={6}>
                            <TextField
                                id="ftp-user" name="user" label="FTP User" value={this.state.user}
                                margin="normal" variant="outlined" fullWidth
                                onChange={this.handleChange}
                            />
                        </Grid>
                        <Grid item xs={6}>
                            <TextField
                                id="ftp-pass" name="pass" type="password" label="Password" value={this.state.pass}
                                margin="normal" variant="outlined" fullWidth
                                onChange={this.handleChange}
                            />
                        </Grid>
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(AddNewFtp));